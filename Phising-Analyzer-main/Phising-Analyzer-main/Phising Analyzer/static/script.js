    ...
    if (file.name.endsWith(".eml")) {
      ...
      reader.onload = function (e) {
        ...
        if (headerEnd !== -1) {
          ...
          var match = headers.match(/client-ip=([^\r\n;]+)/);
          var senderIp = match ? match[1] : "IP not found";

          var match = headers.match(/Received-SPF: ([^\r\n]+)/);
          var spfResult = match ? match[1] : "SPF result not found";

          var dkimMatch = headers.match(/dkim=([^\r\n;]+)/);
          var dkimResult = dkimMatch ? dkimMatch[1] : "DKIM result not found";

          var dmarcMatch = headers.match(/dmarc=([^\r\n;]+)/);
          var dmarcResult = dmarcMatch
            ? dmarcMatch[1]
            : "DMARC result not found";

          var spfResultElement = document.getElementById("spf-result");
          spfResultElement.textContent = "SPF Result: " + spfResult;

          var dkimResultElement = document.getElementById("dkim-result");
          dkimResultElement.textContent = "DKIM Result: " + dkimResult;

          var dmarcResultElement = document.getElementById("dmarc-result");
          dmarcResultElement.textContent = "DMARC Result: " + dmarcResult;
...
function displayAbuseInfo(abuseData, ip) {
  ...
  if (abuseData && abuseData.data) {
    ...
    var suspiciousConfidenceScore =
      abuseData.data.suspiciousConfidenceScore || 0;
